...
data = model_data
obj = obj
xvar <- get(xvar_ch, envir = as.environment(data))
fname <- obj$family[1]
qfun <- paste("q",fname,sep ="")
oxvar <- xvar[order(xvar)]
oyvar <- obj$y[order(xvar)]
...
lc <- length(cent)
centile_data <- data.frame(centile = centile_matrix, order_x = oxvar, order_y = oyvar)
...
area_centiles_plot <- function(
  ...
  model_data$xcol = model_data[[xcol]]
  model_data$ycol = model_data[[ycol]]
  
  # initialize ggplot object
  gg <- ggplot(data = model_data)
  
  if(points) {
    gg <- gg + geom_point(aes(x = xcol, y = ycol),
             size = 1, alpha = 0.05, show.legend = FALSE)
  }
