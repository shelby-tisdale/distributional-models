...
calculate_centiles <- function(
  ...
  demog_combos,
  brain_data,
  race = c("White", "Black", "Asian", "Other", "Hispanic"),
  point_color_sex = TRUE,
  cent = c(97.5, 50, 2.5)
) {
...
for(i in 1:n_models) {
  ...
  ii <- 0
  per <- rep(0, length(cent))
  centile_matrix <- matrix(0, ncol = length(cent), nrow = dim(data)[1])
  ...
  centile_data <- data.frame(
    centile = centile_matrix, order_x = oxvar, order_y = oyvar
  )
  centile_list[[i]] <- centile_data
}

plot_data <- brain_data |>
  filter(sex %in% demog_combos[,1] & race_ethnicity %in% demog_combos[,2])

plot_data$xvar <- plot_data[[xvar_ch]]
plot_data$yvar <- plot_data[[yvar_name]]

area_centiles <- ggplot(plot_data)
if(point_color_sex) {
  area_centiles <- area_centiles +
    geom_point(aes(x = xvar, y = yvar, color = sex), size = 1, alpha = 0.05)
}
else {
  area_centiles <- area_centiles +
    geom_point(aes(x = xvar, y = yvar, color = sex), size = 1, alpha = 0.05)
...
# plot centiles for each race
group <- 1
plot_list = list()
for(group in 1:race) {
  
  current_race <- race[group]
  sex_order <- demog_combos[which(demog_combos[,2] == current_race)]
  race_centiles <- centile_list[which(demog_combos[,2] == current_race)]
  centiles_male <- race_centiles[[which(sex_order == "Male")]]
  centiles_female <- race_centiles[[which(sex_order == "Female")]]
  
  area_centiles <- ggplot(data = brain_data |> filter(race_ethnicity %in% race)) +
  geom_point(aes(x = interview_age, y = area , color = sex))
  
  
  
  ggplot(data = brain_data |> filter(race_ethnicity %in% race)) +
  geom_point(aes(x = interview_age, y = area , color = sex),
             size = 1, alpha = 0.05, show.legend = FALSE) +
  geom_line(data = centile_data_male, aes(x = x, y = c.1),
            size = 1.5, linetype = "dashed", color = "steelblue2") + 
...
# filter for race
# plot centiles for each sex for that race
