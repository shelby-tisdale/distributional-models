...
demog_baseline_wide <- demog_baseline_long |>
  ...
  mutate(
    ...
    41 ~ "Other Hispanic", 
    .default = NA)) |>
  select(src_subject_id, race_1:race_5, hispanic_origin, hispanic_origin_spec)
...
demog_clean <- demog_clean |>
 mutate(
  ...
  select(-starts_with("demo_ethn"), -starts_with("demo_race"), -starts_with("demo_brthdat")) |>
  left_join(demog_baseline_wide, join_by("src_subject_id"))
```

```{r}

table(demog_wide$eventname)
```
