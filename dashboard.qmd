...
format:
  dashboard:
    nav-buttons: [github]
    github: https://github.com/shelby-tisdale/distributional-models
server: shiny
...
area_labels <- read_csv(
  "~/Equitable Data Science REU/distributional-models/Area_Variable_Labels.csv"
  ) |>
  # clean labels
...
demog_brain_combos_thick <- expand.grid(
  ...
  select(sex, race_ethnicity, brain_location, region, thick_label, min_value,
         max_value) |>
  as.matrix(demog_brain_combos_thick)
```
...
output$area_centile_plot_1 <- renderPlot({
  ...
  index <- which(demog_brain_combos_area$sex == sex1 &
                   demog_brain_combos_area$race_ethnicity == race1 &
