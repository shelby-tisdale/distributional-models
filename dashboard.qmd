...
mri_data <- dat.mri |>
  ...
  select(src_subject_id, eventname, smri_thick_cdk_mean, age, sex, site_id_l, race_ethnicity, interview_age, , contains("smri_area_cdk")) |>
  # remove irrelevant ages 
  filter(age < 16 & age > 8) |>
  mutate(interview_age = as.numeric(interview_age)) |>
  rename(thick = smri_thick_cdk_mean) |>
  drop_na(thick, smri_area_cdk_total) |>
  ...
  mutate(sex = first(na.omit(sex)),
         ...
         change_area = smri_area_cdk_total - smri_area_cdk_total[1],
