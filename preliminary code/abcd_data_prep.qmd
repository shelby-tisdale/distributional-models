...
```{r}
#| label: demographic-data-cleaning
...
demog_clean <- demog_clean |>
 mutate(
    ...
    # parent marital status variable
...
thickness <- dat.mri |>
  ...
  select(src_subject_id, eventname, smri_thick_cdk_mean, age, sex, site_id_l, race, race_ethnicity, combined_income, interview_age) |>
  filter(age < 16 & age > 8)
...
# using bamlss code to create simple example
set.seed(841)
# model specification
model1 <- bamlss(
  ...
  data = thickness,
  family = "gaussian",
  # longitudinal vars - doesn't work
  timevar = "age",
...
```{r}
#| label: hierarchical-model-practice
...
sample_ids <- thickness |>
  select(src_subject_id, interview_age, smri_thick_cdk_mean) |>
  drop_na() |>
...
# distribution of cortical thickness in 100 simulated datasets of cortical thickness
thickness |>
  select(src_subject_id, age, smri_thick_cdk_mean) |>
  drop_na() |>
